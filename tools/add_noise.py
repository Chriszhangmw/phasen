...
def generate_mix_list(cwav_list, nwav_list, output_list, snr_range):
    '''
...
if __name__ == '__main__':
    '''
    记录生成数据的方式，
    运行add_noise生成mix文件，但这个文件目前看，没用，然后生成两个文件夹，一个是存放clean的，一个是存放noise的，
    同一个样本的文件名字相同
    所以在执行：
    find ./clean_test_5/ -iname "*.wav" > test.lst   
    将clean或者noise下的文件名获取到即可，
    然后执行cle.py文件替换一下路径就可以了，如果需要加duration时间，执行tool下面的add_duration.py即可
