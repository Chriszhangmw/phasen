...
class PHASEN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # to [B, 2, D, T]
        cmp_spec = torch.cat([cmp_spec[:,:,:self.feat_dim,:],
                                cmp_spec[:,:,self.feat_dim:,:],
                                ],
                                1)
        # to [B, 1, D, T]
        # 强度
        amp_spec = torch.sqrt(torch.abs(cmp_spec[:,0])**2+
                            torch.abs(cmp_spec[:,1])**2,
                        )
        amp_spec = torch.unsqueeze(amp_spec, 1)
        # 强度掩码
        spec = self.amp_conv1(cmp_spec)  #这里就是经过两层卷积网络，和论文一直
        # 相位谱预测
        phase = self.phase_conv1(cmp_spec)   #这里就是经过两层卷积网络，和论文一直
        s_spec = spec
        s_phase = phase
    ...
    def loss(self, est, labels, mode='Mix'):
        ...
        if mode == 'SiSNR':
            ...
            # power compress 
            gth_cprs_mag_spec = (gth_mag_spec + 1e-8)**0.3 #强度压缩
            est_cprs_mag_spec = (est_mag_spec + 1e-8)**0.3
            amp_loss = F.mse_loss(
            ...
            all_loss = amp_loss*0.6 + phase_loss*0.4
